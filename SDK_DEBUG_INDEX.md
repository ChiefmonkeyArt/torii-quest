...
> **Status:** discoverability index (v0.2.201-alpha). A one-page map of the public
...
| `tools/handoffSummary.mjs` (PURE AI-handoff auto-summary, v0.2.190) + `tools/handoff-summary.mjs` CLI (`npm run handoff:summary`) — folds `gatherReleaseReadiness()` into one concise handoff brief for the next agent/model: version / git commit / live URL / gate status / regression+test counts / latest reports / next safe task / key constraints / exact release-verify commands. `buildHandoffSummary`/`formatHandoffSummary`/`formatHandoffSummaryMarkdown` + constants (`HANDOFF_SUMMARY_SCHEMA`='torii.handoff-summary' / v1, `VERIFY_COMMANDS`, `KEY_CONSTRAINTS`, `DEFAULT_NEXT_SAFE_TASK`). `generatedAt` optional/isolated (deterministic tests); null/garbled release → `gate.status:'unknown'`. CLI: text / `--json` / `--markdown` modes, READ-ONLY/local/no-network, never writes without an explicit `--write[=path]`. | `tests/handoff-summary.test.js` |
| `tools/staleDocs.mjs` (PURE stale-doc detector, v0.2.191) + `tools/stale-docs.mjs` CLI (`npm run docs:stale`) — ADVISORY drift detector that catches docs/status/version drift earlier/clearer than the gated docConsistency [14]; REUSES the docConsistency primitives. `detectStaleDocs`/`formatStaleDocs` + `staleVersionHeaderLines`/`testCountsInText`/`reportVersionToken` + `STALE_DOCS_BADGE`. Flags version-HEADER drift per continuity doc, a doc that never mentions the current version, a newest report nobody links, a newest report lagging the current version, and disagreeing test counts across continuity docs. Low false positives (HEADER-only matching + quoted-span stripping). CLI: text / `--json`, READ-ONLY/local/no-network, ALWAYS exits 0 — NOT in `npm run check`. | `tests/stale-docs.test.js` |
