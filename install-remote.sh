...
# A prior install leaves generated build artifacts (public/dashboard.html,
# public/torii-quest-data.json) modified in the working tree, so a plain
# checkout to the pinned tag fails. Force-discard those local changes — they
# are build outputs regenerated by install.sh, and .env is untracked so it is
# never touched.
run_as_build_user git -C "$REPO_DIR" checkout -f "$VERSION" --quiet
