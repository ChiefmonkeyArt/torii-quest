#!/usr/bin/env bash
# torii-quest/install/lib/run.sh — quiet-by-default stage runner.
# Vendored from torii-suite/lib/run.sh (same author, same license) — fully
# generic stage runner, reused as-is.
#
# `run_stage <label> <cmd> [<args>...]` runs a command with:
#   - stdout+stderr streamed to a per-install log file at $QUEST_LOG_FILE
...
#
# Environment:
#   QUEST_QUIET       (default 1)   — set to 0 to stream everything to terminal
#   QUEST_LOG_FILE    (auto-derived) — override to pin log location
#
# Design notes:
...
#   - When QUEST_QUIET=0 (debug mode) we skip the spinner entirely and let the
...
QUEST_QUIET="${QUEST_QUIET:-1}"

if [[ -z "${QUEST_LOG_FILE:-}" ]]; then
  QUEST_LOG_DIR="/var/log/torii-quest"
  # /var/log needs root. Fall back to a per-user tmp dir instead of a raw
  # "mkdir: Permission denied" crash — this file sources before install.sh
  # gets a chance to print its own friendly root-check message, so a script
  # run without sudo should still degrade cleanly rather than blow up here.
  if ! mkdir -p "$QUEST_LOG_DIR" 2>/dev/null || ! chmod 750 "$QUEST_LOG_DIR" 2>/dev/null; then
    QUEST_LOG_DIR="${TMPDIR:-/tmp}/torii-quest-$(id -u 2>/dev/null || echo 0)"
    mkdir -p "$QUEST_LOG_DIR"
    chmod 750 "$QUEST_LOG_DIR" 2>/dev/null || true
  fi
  QUEST_LOG_FILE="${QUEST_LOG_DIR}/install-$(date -u +%Y%m%dT%H%M%SZ).log"
  : > "$QUEST_LOG_FILE"
  chmod 640 "$QUEST_LOG_FILE" 2>/dev/null || true
fi
export QUEST_LOG_FILE
...
#
# In loud mode (QUEST_QUIET=0): prints a plain header, streams everything
# through tee to both terminal and log file, no spinner.
run_stage() {
  local label="$1"; shift
  local start=$SECONDS

  # Announce to the log even in quiet mode so `tail -f $QUEST_LOG_FILE` from
  ...
  tail -n 10 "$QUEST_LOG_FILE" | sed 's/^/    /' >&2
  printf "\n  %sFull log: %s%s%s\n\n" "$UI_DIM" "$UI_RESET" "$QUEST_LOG_FILE" "" >&2
  exit "$rc"
}
