...
# ui_banner <version> — the very first thing the operator sees. Quest's own
# wordmark, trimmed from torii-suite's multi-app banner set since this
# installer only ever shows Quest's — shipping the others was dead weight.
...
ui_banner() {
  ...
  printf "%s  the federated metaverse   %s%s%s\n"       "$UI_DIM" "$UI_CYAN2" "${version}" "$UI_RESET"
  printf "\n"
}
...
# --------------------------------------------------------------------------- #
# Prompts — prefer /dev/tty, fall back to stdin/stdout                       #
# --------------------------------------------------------------------------- #

# Prompt I/O target — prefer a real controlling terminal so prompts still
# work when stdin is a pipe (e.g. `curl ... | bash`, which is how most
# people will actually run this). Fall back to stdin/stdout when no /dev/tty
# is reachable at all (containers, CI, background/non-interactive runs) so
# a missing tty degrades to reading whatever stdin has instead of a hard
# `set -e` exit with no explanation.
# A permission check (-r/-w) is not enough: /dev/tty can exist with wide-open
# perms yet still fail to open with ENXIO when the process has no controlling
# terminal at all (curl-pipe-to-bash, cron, CI, a detached SSH command). Only
# an actual open proves it's usable.
...
ui_ask() {
  ...
  if [[ -n "$default" ]]; then
    printf "  %s%s%s %s %s[%s]%s: " \
      "$UI_PINK" "$UI_ARROW" "$UI_RESET" "$prompt" "$UI_DIM" "$default" "$UI_RESET"
  else
    printf "  %s%s%s %s: " "$UI_PINK" "$UI_ARROW" "$UI_RESET" "$prompt"
  fi
  # Prefer a real controlling terminal so this still works when stdin is a
  # pipe (curl-pipe-to-bash). Without one, read from the inherited stdin
  # (fd 0) directly rather than reopening /dev/stdin by redirect — on some
  # bash builds a fresh redirect onto a pipe triggers a full-buffer
  # read-ahead whose unconsumed remainder gets flushed as literal NULs.
  if [[ "$_UI_HAS_TTY" == "1" ]]; then
...
ui_confirm() {
  local reply
  printf "\n  %s%s%s %s %s[y/N]%s " \
    "$UI_PINK" "$UI_ARROW" "$UI_RESET" "$*" "$UI_DIM" "$UI_RESET"
  if [[ "$_UI_HAS_TTY" == "1" ]]; then
    read -r reply < /dev/tty
  else
    read -r reply
