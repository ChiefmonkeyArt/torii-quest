...
> Current version: **v0.2.181-alpha** | Live: [torii-quest.pplx.app](https://torii-quest.pplx.app)
...
| Metric | Value |
|---|---|
| Source version | **v0.2.181-alpha** (build truth; live trails — manual maintainer deploy) |
| Tests | **912 passing / 63 files** (profiles: `test:fast` ~5 files, `test:foundation` ~20 files) |
...
| Active slice | v0.2.181 LEAN-2 in-world gateway PORTAL TRIGGER — proximity tick arms the injected boundary + raises a HUD prompt (both inert); an explicit KeyF interact is the ONLY navigating step (confirmed same-origin `/zone` hop). Window injected only at the `main.js` boundary; allowlist scoped `['/zone/']` (never `['/']`) |
