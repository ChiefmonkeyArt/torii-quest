...
> Current version: **v0.2.182-alpha** | Live: [torii-quest.pplx.app](https://torii-quest.pplx.app)
...
| Metric | Value |
|---|---|
| Source version | **v0.2.182-alpha** (build truth; live trails — manual maintainer deploy) |
| Tests | **940 passing / 64 files** (profiles: `test:fast` ~5 files, `test:foundation` ~21 files) |
...
| Active slice | v0.2.182 LEAN-2 pure SPA `/zone/<slug>` ROUTE PARSER (`zoneRoute.js`) — classifies a same-origin path as home/zone/invalid, validates the slug strictly, maps a valid zone to an INERT display state (HUD notice). `safeRoutePath` gate rejects dot-dot/percent/protocol-relative/scheme/markup/over-length; `window.location` read + `popstate` wired ONLY at the `main.js` boundary. NO network/relay/nav/sign/publish. Hard-refresh deep-link needs a static-host SPA fallback (serve `index.html` for `/zone/*`) — documented, not faked |
