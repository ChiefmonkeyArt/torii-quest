...
const SERVER_VERSION = 'v0.2.815-alpha';
...
// Bot milestone chunk 2 (v0.2.815-alpha): server-authoritative bots.
...
// v0.2.815-alpha (UPD-2): admin-gated "Update Now". QUEST_ADMIN_NPUB accepts an
...
// ADR-0094 (v0.2.815-alpha): server-side always-on presence beacon. The server
...
// ADR-0032 (v0.2.815-alpha): server-side truth for "is this session the owner,
...
// v0.2.815-alpha (Bug C): 15 minutes of no gameplay activity (see the PING/PONG
...
// v0.2.815-alpha: live in-arena leaderboard. In addition to the on-close SCORE
...
// Session-token authority (v0.2.815-alpha). Login signs a NIP-98 event ONCE
...
// v0.2.815-alpha (UPD-2): admin-update request authority. Writes atomic request
...
// ADR-0094 (v0.2.815-alpha): server-side always-on presence beacon authority.
...
// ADR-0018 (v0.2.815-alpha): let arenaBotSim.spawn() use its env-driven default
...
function closeSession(sess, reason) {
  ...
  if (sessions.has(sess.id)) {
    ...
    // v0.2.815-alpha: RETIRE (not drop) so a disconnected player stays on the
...
function finishAuth(sess, { npub, pubkey, character }) {
  ...
  sess.npub = npub;
  sess.pubkey = pubkey;
  // v0.2.815-alpha: accept a client-sent character key (validated against the
  // known set) OR a 64-hex Character Forge mesh hash. v0.2.815-alpha: an absent/
...
async function handleMessage(sess, raw) {
  ...
  // v0.2.815-alpha (Bug C): only gameplay messages count as "activity" for the
  ...
  // --- Handshake phase ---
  if (!sess.authed) {
    // v0.2.815-alpha: bearer-token auth (login signed once via NIP-98). No
...
function _logShotResolve(shooterId, shotMsg, peerCount, result, botResult, decision, srvNow, rewindTs) {
  ...
  // v0.2.815-alpha: surface the SERVER-time rewind inputs — the client-reported
  ...
  // v0.2.815-alpha: for the nearest bot, log its CURRENT vs REWOUND XZ position —
...
function resolveAndBroadcast(shooter, shotMsg) {
  ...
  // v0.2.815-alpha: bots rewind to the SAME server-time rewindTs the peer
  ...
  if (outcome.killed) {
    ...
    // v0.2.815-alpha: push the updated standings immediately on a kill so the
...
if (BOT_SIM_ENABLED) {
  setInterval(() => {
    ...
    const now = Date.now();
    // v0.2.815-alpha: record post-tick bot positions for lag-compensated
...
// v0.2.815-alpha: periodic live SCORE broadcast. broadcastScoreFrame() is a
...
const httpServer = createServer((req, res) => {
  ...
  // v0.2.815-alpha session-token endpoints (plain HTTP, same origin as /mp).
  ...
  // v0.2.815-alpha (UPD-2) admin-update endpoints.
  ...
  // v0.2.815-alpha: PUBLIC read. Deploy restarts arena-ws, which drops in-memory
  ...
  // ADR-0094 (v0.2.815-alpha): server-side always-on presence beacon.
  ...
  // v0.2.815-alpha (ADR-0025) Kami Mode ema intake. Admin-gated by session token.
  ...
  // v0.2.815-alpha (ADR-0039) Kami replies read. Admin-gated: only the logged-in
  ...
  // v0.2.815-alpha (real text-to-3d, Step C of ADR-0091): generate a human-
  ...
  // Paid character-creation (LNURL-pay + NIP-57, v0.2.815-alpha): two-phase.
  ...
  // v0.2.815-alpha (headless-fp-bodies for custom meshes): author a headless FP
