...
const SERVER_VERSION = 'v0.2.716-alpha';
...
// Bot milestone chunk 2 (v0.2.716-alpha): server-authoritative bots.
...
// v0.2.716-alpha (UPD-2): admin-gated "Update Now". QUEST_ADMIN_NPUB accepts an
...
// ADR-0032 (v0.2.716-alpha): server-side truth for "is this session the owner,
...
// v0.2.716-alpha: valid character keys the client may send in AUTH/AUTH_TOKEN.
...
// v0.2.716-alpha: live in-arena leaderboard. In addition to the on-close SCORE
...
// Session-token authority (v0.2.716-alpha). Login signs a NIP-98 event ONCE
...
// v0.2.716-alpha (UPD-2): admin-update request authority. Writes atomic request
...
// ADR-0018 (v0.2.716-alpha): let arenaBotSim.spawn() use its env-driven default
...
function closeSession(sess, reason) {
  ...
  if (sessions.has(sess.id)) {
    ...
    // v0.2.716-alpha: RETIRE (not drop) so a disconnected player stays on the
...
function finishAuth(sess, { npub, pubkey, character }) {
  ...
  sess.npub = npub;
  sess.pubkey = pubkey;
  // v0.2.716-alpha: accept client-sent character key (validated against the
...
async function handleMessage(sess, raw) {
  ...
  // --- Handshake phase ---
  if (!sess.authed) {
    // v0.2.716-alpha: bearer-token auth (login signed once via NIP-98). No
...
function _logShotResolve(shooterId, shotMsg, peerCount, result, botResult, decision, srvNow, rewindTs) {
  ...
  // v0.2.716-alpha: surface the SERVER-time rewind inputs — the client-reported
  ...
  // v0.2.716-alpha: for the nearest bot, log its CURRENT vs REWOUND XZ position —
...
function resolveAndBroadcast(shooter, shotMsg) {
  ...
  // v0.2.716-alpha: bots rewind to the SAME server-time rewindTs the peer
  ...
  if (outcome.killed) {
    ...
    // v0.2.716-alpha: push the updated standings immediately on a kill so the
...
if (BOT_SIM_ENABLED) {
  setInterval(() => {
    ...
    const now = Date.now();
    // v0.2.716-alpha: record post-tick bot positions for lag-compensated
...
// v0.2.716-alpha: periodic live SCORE broadcast. broadcastScoreFrame() is a
...
const httpServer = createServer((req, res) => {
  ...
  // v0.2.716-alpha session-token endpoints (plain HTTP, same origin as /mp).
  ...
  // v0.2.716-alpha (UPD-2) admin-update endpoints.
  ...
  // v0.2.716-alpha: PUBLIC read. Deploy restarts arena-ws, which drops in-memory
  ...
  // v0.2.716-alpha (ADR-0025) Kami Mode ema intake. Admin-gated by session token.
  ...
  // v0.2.716-alpha (ADR-0039) Kami replies read. Admin-gated: only the logged-in
