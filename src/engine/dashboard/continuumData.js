...
export const CONTINUUM_VERSION = 'v0.2.189-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.189-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1032 passing / 68 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.189 release-readiness JSON EXPORT (tooling/docs only, no runtime change) — the local ship verdict is now machine-readable: a pure buildReleaseStatusJson() wraps the existing releaseReadiness summary in a stable schema envelope (schema/schemaVersion + status, ready, blockers, unknowns, per-signal map, version/commit) so the dashboard/handoff/updater/agents can consume readiness without parsing human terminal text. Deterministic by design: generatedAt is the only non-deterministic field and is optional/isolated (omit → null for reproducible tests; CLI stamps a real ISO time). Garbled input → honest {status:unknown, error:no-summary}; never throws, never mutates source. New npm run release:status:json + node tools/release-readiness.mjs --json (canonical machine call — raw stdout is pure JSON; for scripted npm use `npm run --silent release:status:json`, since plain `npm run` prepends a lifecycle banner to stdout). +7 unit tests. NON-GOALS held: no fs writes from the game, no network, no deploy/publish, no gameplay/portal/physics/controls/Nostr change.' },
  ],
