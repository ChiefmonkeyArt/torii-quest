...
export const CONTINUUM_VERSION = 'v0.2.190-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  metrics: [
    ...
    { label: 'Active slice', value: 'v0.2.190 handoff AUTO-SUMMARY (tooling/docs only, no runtime change) — a new pure buildHandoffSummary() folds the existing local status/readiness inputs into ONE concise brief for the next agent/model: version, git commit, live URL, the current gate verdict (consumed from the existing releaseReadiness summary — no re-implementation), regression count + test-profile counts, latest reports, the recommended next SAFE task, the standing key constraints, and the exact release-verify commands. Thin CLI npm run handoff:summary prints text by default, with --json (stable schema torii.handoff-summary v1), --markdown, and an opt-in --write[=path] (the ONLY thing that writes — read-only otherwise). Deterministic: generatedAt is the only non-deterministic field (optional/isolated; omit → null for tests). Garbled/missing release input → honest unknown gate; never throws. +13 unit tests. NON-GOALS held: no fs writes from the game, no network, no deploy/publish, no gameplay/portal/physics/controls/Nostr change.' },
  ],
