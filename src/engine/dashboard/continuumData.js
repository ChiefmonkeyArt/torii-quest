...
export const CONTINUUM_VERSION = 'v0.2.196-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.196-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1197 passing / 75 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.196 UPDATE FLOW SMOKE (infra/testing/tooling only, no runtime change) — a pure, node-safe read-only smoke harness (engine/update/updateFlowSmoke.js) that folds the shipped update-check / release-metadata / consent contracts into ONE fail-fast report so future torii.quest / VPS self-update work can be regression-checked locally without a browser, shell, package manager, or network: current version read; release metadata shape parses; a newer release classifies as update-available; a same/older release classifies as up-to-date; malformed/draft/empty payloads degrade to unknown without throwing; the status panel/view are manual-only (readOnly, actionable:false); the release-metadata safety floor REJECTS a tampered autoUpdate:true; none of the read-only outputs expose a fetch/install/exec/apply/spawn callable; apply-update is confirmation-gated (no grant → blocked, grant → allowed but STILL never performed); and every exercised report pins performed/actionable/autoUpdate/installed/executed/fetched/network/signed/published/navigated=false. It composes only the already-pure helpers over deterministic LOCAL fixtures — it fetches/installs/applies NOTHING and never reaches the wire. Surfaced via the SDK (updateFlowSmoke, experimental), the debug shell (ToriiDebug.shells.updateFlowSmoke), and folded into buildShellReport. +17 unit tests. NON-GOALS held: NOT an updater — performs no real update; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
  ...
  completed24h: [
    ...
    'v0.2.194 — NOSTR READ-PATH HEALTH (tooling/dashboard only, no runtime change): a pure, node-safe read-only health model (engine/nostr/readHealth.js) folding the shipped read-path proofs (relay read model, no-EVENT publish verb, kind:0 profile read, kind-30000 leaderboard read, write-paths consent-gated, SEC-1/2/3 future-gated) into ONE report confirming the Nostr surface is still READ-ONLY at the MVP stage and every live-write path stays gated behind explicit consent. Exercises only the already-pure read helpers over deterministic LOCAL sample events + reads the consent registry — NO relay I/O, NO WebSocket, NO signing, NO publishing, NO NIP-07, NO key handling, NO network. Surfaced via SDK (nostrReadHealth), debug shell (ToriiDebug.shells.readHealth), and the Continuum panel. +16 unit tests. NON-GOALS: no live Nostr writes/signing/EVENT publishing/relay network/user-key handling; no gameplay/portal/physics/shooting/controls change.',
    'v0.2.193 — VPS INSTALL DRY-RUN (tooling/docs only, no runtime change): a local, read-only readiness checklist an operator runs BEFORE deploying torii.quest to a VPS/static host — NO SSH, network, DNS, or server change. Pure runVpsDryRun() folds 11 checks: required deploy docs present; dist/ (if built) carries index.html (+ the copied release-metadata.json); public/release-metadata.json present and manual-only/non-actionable (reuses validateReleaseMeta()); metadata + UPDATE_CHECK.md point at the real repo ChiefmonkeyArt/torii-gate; the /zone/* SPA fallback documented (reuses fallbackEvidence()); VPS_INSTALL.md build/manual-update/rollback/security sections; build/verify commands documented; rollback + manual/no-auto-update wording; service-worker stance documented; live URL references clear. Thin CLI npm run vps:dry-run (text/--json) exits non-zero ONLY on a blocking FAIL (warn/skip never fail). Local/read-only/network-free. +43 unit tests. NON-GOALS: no deploy, no SSH/DNS/server change, no gameplay/portal/physics/shooting/controls or live Nostr write change.',
    'v0.2.192 — GITHUB RELEASE METADATA (tooling/docs only, no runtime change): prepares the static metadata a FUTURE torii.quest / VPS update-checker reads to show an inert "update available" notice, with NO live update and NO runtime network. Pure buildReleaseMeta() shapes channel (from the version tag)/version/commit/doc-only GitHub source URLs/dist artifact expectations/requiredFiles+Checks/manual+no-auto-update consent wording; validateReleaseMeta() enforces the safety floor (update.autoUpdate + actionable MUST be false → ERROR otherwise). Thin CLI npm run release:meta (text/--json/--write the deterministic public/release-metadata.json/--stamp for live provenance), local/read-only/network-free, ALWAYS exits 0. +23 unit tests. NON-GOALS: no live update execution, no gameplay/portal/physics/shooting/controls or live Nostr write change.',
  ],
