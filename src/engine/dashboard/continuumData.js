...
export const CONTINUUM_VERSION = 'v0.2.243-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.243-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.243 ZONE RENDERABLE-ROUTE REPAIR (game slice) — fixes the v0.2.242 follow-up: the live fetch saw the HTML, but interactive Playwright got "page.goto: Download is starting" opening /zone/plebeian-market-bazaar. ROOT CAUSE: the static host (torii-quest.pplx.app) infers Content-Type from file EXTENSION, so the v0.2.242 EXTENSIONLESS file dist/zone/<slug> was served as application/octet-stream — a real browser DOWNLOADS it instead of rendering (curl saw raw bytes; the browser respects the MIME). FIX (no backend): canonical zone route is now the TRAILING-SLASH /zone/<slug>/, served by the directory-index shell dist/zone/<slug>/index.html. The host DOES resolve a trailing-slash URL onto its directory index (the same resolution that serves root /), and a .html file is served as renderable text/html. The pure planner (tools/zoneShells.mjs) now emits zone/<slug>/index.html; zoneRouteFor + handoffRouteFor build the trailing-slash route; the parser accepts BOTH /zone/<slug> and /zone/<slug>/ (normalising route output to the canonical trailing slash) so an in-app hop or a user typing the bare path still resolves client-side — only a COLD no-slash deep-link degrades. The zone-fallback readiness guard (zoneFallbackReadiness checkDistRoutes + zones:check + regression [15] + build-continuum) was reconciled to ALLOW the verified directory-index shell (/zone/<slug>/index.html byte-identical to index.html) while still failing on any other /zone/* file that would shadow the fallback. RESIDUAL RISK (needs live re-smoke by the parent): a cold no-slash /zone/<slug> hit (no trailing slash) still depends on host default behaviour — unverifiable locally. Preserves the v0.2.240 service-worker fail-soft precache (zone shells are HTML → network-first), the v0.2.238 fail-closed loop, and the v0.2.236 NIP-07 login decoupling; root entry flow + ESC pause unchanged. Prior — v0.2.242 zone exact-path extensionless shell (downloaded as octet-stream, superseded); v0.2.241 zone hard-refresh shell (directory-index, superseded); v0.2.240 travel-gateway entry repair; v0.2.239 travel-gateway placement; v0.2.238 ENTER-ARENA reset-crash fix. HARD CONSTRAINTS held: godMode false; no new timers (loop uses rAF only); no new hot-path Vector3/Matrix4; nostrich comments; Chiefmonkey exact; debug tools ship unconditionally; non-religious ethics guard + useful-job invariant intact; no Nostr writes/signing beyond the existing login/read; no deploy/publish/push (parent handles those).' },
  ],
