...
export const CONTINUUM = Object.freeze({
  ...
  leanRoute: [
    ...
    { id: 'LEAN-5', state: 'in-progress', progress: 55, slice: 'torii.quest GitHub update-check', status: 'helper + view-model + release source/status + static release-metadata template/spec (v0.2.192, npm run release:meta) + local install dry-run checklist (v0.2.193, npm run vps:dry-run); needs read-only releases fetch + prompt mesh' },
  ],

  // Now / Next / Later.
  activeNow: [
    'v0.2.193 — VPS INSTALL DRY-RUN (tooling/docs only, no runtime change): a local, read-only readiness checklist an operator runs BEFORE deploying torii.quest to a VPS/static host — with NO SSH, network, DNS, or server change. Pure runVpsDryRun() (tools/vpsDryRun.mjs) folds 11 checks: required deploy docs present; dist/ (if built) carries index.html (+ the copied release-metadata.json); public/release-metadata.json present and manual-only/non-actionable (reuses validateReleaseMeta()); metadata + UPDATE_CHECK.md point at the real repo ChiefmonkeyArt/torii-gate; the /zone/* SPA fallback is documented (reuses fallbackEvidence()); VPS_INSTALL.md carries the build/manual-update/rollback/security sections; the npm run build / npm run check commands are documented; rollback + manual/no-auto-update wording present; the service-worker stance is documented; and live URL references are clear. Thin CLI npm run vps:dry-run (text default; --json) reads local files only and prints a clear pass/fail/warn/skip checklist; it exits non-zero ONLY on a blocking FAIL (warnings + the skipped dist/ row never fail). Local/read-only/network-free. +43 unit tests. NON-GOALS held: no deploy, no SSH/DNS/server change, no gameplay/portal/physics/shooting/controls or live Nostr write change.',
