...
export const CONTINUUM_VERSION = 'v0.2.201-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.201-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.201 MVP RELEASE-CANDIDATE GATE (infra/tooling only, no runtime change) — a pure/local gate (tools/mvpRcGate.mjs + CLI tools/mvp-rc-gate.mjs, npm run rc:gate) that answers ONE question: is this build ready to be called an MVP proof-of-concept RELEASE CANDIDATE? It COMPOSES the existing readiness verdicts — the MVP readiness rollup (9 live + injected signals) and the release-readiness summary (version sync, test profiles, regression gate, bundle advisory, /zone/* fallback, docs consistency) — into one concise READY / NEAR / BLOCKED verdict + a blended percentage + the blocking reasons + the next one or two safe tasks. It creates NO release: no git tag, no GitHub release, no deploy, no publish, no upload — read-only oversight that PREVIEWS npm run test:release. +14 unit tests. NON-GOALS held: read-only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
  ...
  // Now / Next / Later.
  activeNow: [
    'v0.2.201 — MVP RELEASE-CANDIDATE GATE (infra/tooling only, no runtime change): a pure/local gate (tools/mvpRcGate.mjs + CLI tools/mvp-rc-gate.mjs, npm run rc:gate) that answers whether this build is ready to be called an MVP proof-of-concept RELEASE CANDIDATE. It COMPOSES the already-computed readiness verdicts — the MVP readiness rollup (9 live + injected signals) + the release-readiness summary (version sync, test profiles, regression gate, bundle advisory, /zone/* fallback, docs consistency) — into ONE concise READY / NEAR / BLOCKED verdict with a blended percentage, the blocking reasons, and the next one or two safe tasks. A single short MVP signal is a NEAR; a failing required release signal (or two+ MVP misses) is BLOCKED; both fully green with nothing unchecked is READY. It re-derives no check and creates NO release: no git tag, no GitHub release, no deploy, no publish, no upload, no network — read-only oversight that PREVIEWS npm run test:release. +14 unit tests. NON-GOALS held: read-only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
  ...
  completed24h: [
    'v0.2.200 — DASHBOARD METRIC FRESHNESS CLEANUP (infra/dashboard data-quality only, no runtime change): resolved the recurring stale LAST-KNOWN test-count flagged in security reviews — HEALTH_LASTKNOWN.totalTests was a SECOND hand-maintained copy stuck at "1180 passing" while the "at a glance" Tests metric had moved on, so the page disagreed with itself and the MVP percentage could be read off a stale number. A single curated source of truth (CURRENT_TEST_STATUS) + a testCountLabel() helper now feed BOTH displayed surfaces, so they can no longer drift apart. New unit tests assert the curated FILE count equals the real on-disk test-file count (an added/removed test file FAILS the suite until the constant is bumped), that both labels derive from the one source, and that the dashboard capture agrees with the MVP-rollup capture (mvpReadiness.DEFAULT_TEST_STATUS). NON-GOALS held: read-only data cleanup; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
    'v0.2.199 — AGENT HANDOFF READINESS EXPORT (infra/docs/tooling only, no runtime change): a pure, node-safe export (tools/agentHandoff.mjs + CLI tools/agent-handoff.mjs, npm run handoff:agent) that folds the EXISTING local status signals a NEXT agent — including non-Perplexity tools (DeepSeek/Perplexica/Routstr-style handoffs) — needs to continue the safe MVP pipeline WITHOUT reading the whole repo: version, live URL, gate verdict, regression + test-profile counts, latest reports, the standing hard constraints, the next SAFE task, the pure smoke-harness inventory, and the v0.2.198 MVP-readiness rollup (pct + status). COMPOSES the existing buildHandoffSummary() brief + runMvpReadiness() rollup rather than re-deriving either, adding only the smoke-harness inventory + readiness pct/status the base summary lacked. Text/JSON/markdown modes; --write emits HANDOFF.generated.md (the ONLY write, confined inside the repo via the shared resolveHandoffWritePath) and NEVER touches the curated HANDOFF.md. +13 unit tests. NON-GOALS held: read-only except the explicit --write output; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
    'v0.2.198 — MVP READINESS ROLLUP (infra/dashboard/tooling only, no runtime change): a pure, node-safe read-only rollup (engine/status/mvpReadiness.js) that folds the already-pure local readiness signals into ONE verdict with an MVP percentage/status + next safe task, so the user sees how close the read-only MVP proof is without manually digging through every harness, doc, and gate. 9 signals: version marker valid; Nostr read-path health (runReadHealth); gateway travel smoke (runGatewayTravelSmoke); update-flow smoke (runUpdateFlowSmoke); host-route smoke (runHostRouteSmoke); release-metadata safety floor (validateReleaseMeta rejects tampered autoUpdate); injected last-known test-suite verdict; injected VPS manual-deploy dry-run verdict; injected docs/handoff freshness verdict. The four live smoke verdicts come from the already-pure harnesses; the fs-backed signals (tests/VPS/docs) are INJECTED via opts with curated last-known defaults (like the dashboard ship/health models) so the module stays pure. Every report pins served/deployed/published/navigated/performed/fetched/wrote/network=false. Surfaced via SDK (mvpReadiness, experimental), debug shell (ToriiDebug.shells.mvpReadiness), and buildShellReport. +14 unit tests. NON-GOALS held: read-only — serves/deploys/fetches/writes NOTHING; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
    'v0.2.197 — HOST ROUTE SMOKE (infra/testing/tooling only, no runtime change): a pure, node-safe read-only smoke harness (engine/host/hostRouteSmoke.js) that folds the shipped static-host readiness contracts into ONE fail-fast report so future torii.quest / VPS static-host work can be regression-checked locally without a server, shell, DNS, SSH, or network. 10 signals: root index.html present; DIST_SPEC artifacts (index.html + assets/) present; continuum dashboard page + data JSON present; release-metadata.json present AND manual-only/non-actionable; release REQUIRED_FILES floor intact; /zone/* SPA fallback documented in VPS_INSTALL.md/HANDOFF.md; no built file shadows the /zone/* fallback; unknown /zone/<slug> is NOT a built file (→ host serves index.html) while the parser keeps the slug safe; route parser classes a good slug ZONE and every hostile path INVALID; no host-side action (every report pins served/deployed/navigated/performed/external/network/wrote/fetched=false). Inspects deterministic LOCAL fixtures (built-path list + doc text) only — touches NO server/DNS/SSH/network, deploys NOTHING. Surfaced via SDK (hostRouteSmoke, experimental), debug shell (ToriiDebug.shells.hostRouteSmoke), and buildShellReport. +17 unit tests. NON-GOALS held: NOT a VPS deployment — no real server touched; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
  ],
