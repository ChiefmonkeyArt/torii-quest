...
export const CONTINUUM_VERSION = 'v0.2.191-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.191-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1077 passing / 70 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.191 STALE-DOC DETECTOR (tooling/docs only, no runtime change) — a new ADVISORY detector that catches docs/status/version drift earlier and more clearly than the gated docConsistency guard. Pure detectStaleDocs() flags precise version-HEADER drift in each continuity doc, a continuity doc that never mentions the current version, a newest report no continuity doc links, a newest report that lags the current version, and disagreeing test counts across the continuity docs. Low false positives by design: it matches HEADER assertions (only separator/markup chars between "version" and the marker) and strips backtick/double-quote spans so changelog prose is ignored. Thin CLI npm run docs:stale (text default; --json) is local/read-only/network-free and ALWAYS exits 0 — advisory, NOT in npm run check (the hard gate stays docConsistency [14]). +25 unit tests. NON-GOALS held: no gameplay/portal/physics/shooting/controls or live Nostr write change.' },
  ],
  ...
  // Now / Next / Later.
  activeNow: [
    'v0.2.191 — STALE-DOC DETECTOR (tooling/docs only, no runtime change): a new ADVISORY detector that catches docs/status/version drift earlier and more clearly than the gated docConsistency guard. Pure detectStaleDocs() (tools/staleDocs.mjs) reuses the docConsistency primitives and flags precise version-HEADER drift in each continuity doc, a continuity doc that never mentions the current version, a newest report no continuity doc links to, a newest report that lags the current version, and disagreeing test counts across the continuity docs. Low false positives by design: a HEADER assertion requires only separator/markup chars between "version" and the marker, and backtick/double-quote spans are stripped so changelog prose ("version drift fixed (v0.2.137)") and quoted examples are ignored. Thin CLI npm run docs:stale (text default; --json) is local/read-only/network-free and ALWAYS exits 0 — advisory, NOT wired into npm run check (the hard gate stays docConsistency [14], which keeps false positives from blocking safe dev). +25 unit tests. NON-GOALS held: no gameplay/portal/physics/shooting/controls or live Nostr write change.',
  ...
  completed24h: [
    'v0.2.191 — STALE-DOC DETECTOR (tooling/docs only, no runtime change): a new ADVISORY detector catching docs/status/version drift earlier/clearer than the gated docConsistency guard. Pure detectStaleDocs() flags version-HEADER drift per continuity doc, a doc that never mentions the current version, a newest report nobody links, a newest report lagging the current version, and disagreeing test counts across continuity docs. Low false positives: HEADER-only matching (separator/markup gap) + quoted-span stripping so changelog prose is ignored. Thin CLI npm run docs:stale (text/--json), local/read-only/network-free, ALWAYS exits 0 — advisory, NOT in npm run check (hard gate stays docConsistency [14]). +25 unit tests. NON-GOALS: no gameplay/portal/physics/shooting/controls or live Nostr write change.',
    'v0.2.190 — handoff AUTO-SUMMARY (tooling/docs only, no runtime change): a new pure buildHandoffSummary() folds the existing local status/readiness inputs into ONE concise brief for the next agent/model — version, git commit, live URL, the current gate verdict (consumed from the releaseReadiness summary via gatherReleaseReadiness(); no check re-implemented), regression + test-profile counts, latest reports, the recommended next SAFE task, the standing key constraints, and the exact release-verify commands. Thin CLI npm run handoff:summary (text default; --json schema torii.handoff-summary v1; --markdown; opt-in --write[=path] is the ONLY writer). Deterministic generatedAt (optional/isolated); garbled/missing release → honest unknown gate; never throws. +13 unit tests. NON-GOALS: no fs writes from the game, no network, no deploy/publish, no gameplay/portal/physics/controls/Nostr change.',
    'v0.2.189 — release-readiness JSON EXPORT (tooling/docs only, no runtime change): the local ship verdict is now machine-readable. A pure buildReleaseStatusJson() wraps the existing releaseReadiness summary in a stable schema envelope (schema/schemaVersion + status, ready, blockers, unknowns, per-signal map, version/packageVersion/gitCommit, latestReports) so the dashboard/handoff/updater/agents can consume readiness without parsing human terminal text. Deterministic by design: generatedAt is the ONLY non-deterministic field and is optional + isolated (omit → null for reproducible tests; the CLI stamps a real ISO time). Garbled/non-object input → honest {status:unknown, error:no-summary}; never throws, never mutates source. New npm run release:status:json + node tools/release-readiness.mjs --json (canonical machine call — raw stdout is pure JSON; scripted npm use `npm run --silent release:status:json`, since plain `npm run` prepends a lifecycle banner to stdout). +7 unit tests. NON-GOALS: no fs writes from the game, no network, no deploy/publish, no gameplay/portal/physics/controls/Nostr change.',
