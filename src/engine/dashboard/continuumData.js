...
export const CONTINUUM_VERSION = 'v0.2.199-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.199-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1241 passing / 78 files (profiles: test:fast ~5, test:foundation ~25)' },
  ...
  completed24h: [
    ...
    'v0.2.196 — UPDATE FLOW SMOKE (infra/testing/tooling only, no runtime change): a pure, node-safe read-only smoke harness (engine/update/updateFlowSmoke.js) that folds the shipped update-check / release-metadata / consent contracts into ONE fail-fast report so future torii.quest / VPS self-update work can be regression-checked locally without a browser, shell, package manager, or network. 10 signals: current version read; release metadata shape parses; newer release → update-available; same/older release → up-to-date; malformed/draft/empty → unknown (no throw); status panel/view manual-only (readOnly, actionable:false); release-metadata safety floor REJECTS tampered autoUpdate:true; no fetch/install/exec/apply/spawn callable on any output; apply-update confirmation-gated (no grant → blocked, grant → allowed but never performed); no auto action (every report pins performed/actionable/autoUpdate/installed/executed/fetched/network/signed/published/navigated=false). Composes only the already-pure helpers over deterministic LOCAL fixtures — fetches/installs/applies NOTHING, never reaches the wire. Surfaced via SDK (updateFlowSmoke, experimental), debug shell (ToriiDebug.shells.updateFlowSmoke), and buildShellReport. +17 unit tests. NON-GOALS held: NOT an updater — no real update performed; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
    'v0.2.195 — GATEWAY TRAVEL SMOKE (infra/testing/tooling only, no runtime change): a pure, node-safe read-only smoke harness (engine/gateway/travelSmoke.js) that folds the shipped travel-flow contracts into ONE fail-fast report so future portal/travel work can be regression-checked locally without a browser. 10 signals: trigger arms on proximity; proximity ALONE never navigates; explicit confirm required; same-origin /zone/<slug> route only; allowlist scoped (never "/"); valid /zone/<slug> resolves; hostile routes (traversal/scheme/protocol-relative) rejected; no external gateway website carried; consent gates travel; no auto travel/write (every report pins navigated/performed/external/signed/published/network=false). Drives the boundary with dryRun:true + NO injected transport, so a fully-confirmed confirm() is a dry-run no-op that navigates NOTHING. Surfaced via SDK (travelSmoke, experimental), debug shell (ToriiDebug.shells.travelSmoke), and buildShellReport. +12 unit tests. NON-GOALS held: no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
  ],
