...
export const CONTINUUM_VERSION = 'v0.2.206-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.206-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.206 MVP RELEASE PACKAGE INDEX (docs/tooling only, no runtime change) — a single discoverability INDEX (tools/releasePackage.mjs + CLI tools/release-package.mjs, npm run release:package, default MVP_RELEASE_PACKAGE.md) that points humans and future agents at every MVP-proof package file (release notes draft, playtest checklist + results template, handoff briefs, progress/todo, update/VPS/zone-fallback readiness) plus current version/commit/test-count/live URL, known advisories, and the recommended next safe action. Stat-s each indexed file for present/missing; emits text / --json / --markdown plus an opt-in bounded in-repo --write (confined via the shared resolveHandoffWritePath boundary). INDEX ONLY: no GitHub release/tag/announce/network/deploy/publish. +12 unit tests. NON-GOALS held: read-only except the explicit --write output; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
  ...
  completed24h: [
    ...
    'v0.2.202 — MVP PROOF RELEASE NOTES DRAFT (docs/tooling only, no runtime change): a pure/local release-notes draft (tools/releaseNotes.mjs + CLI tools/release-notes.mjs, npm run release:notes) for the first MVP proof-of-concept candidate. It COMPOSES the curated "what\'s built" narrative — shooter proof loop, Nostr read/profile/leaderboard proof surfaces, gateway travel shell, update/VPS readiness, Continuum dashboard, SDK/debug handoff surfaces, tests/guardrails, known non-blocking advisories — with the already-computed local verdicts (the MVP readiness rollup + the release-candidate gate + the handoff brief) and emits a DRAFT in text / --json / --markdown, plus an opt-in bounded in-repo --write (default RELEASE_NOTES_DRAFT.md, confined via the shared resolveHandoffWritePath boundary). It re-derives no check and DRAFT ONLY: no GitHub release, no git tag, no public announcement, no network/server. +13 unit tests. NON-GOALS held: read-only except the explicit --write output; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
  ],
