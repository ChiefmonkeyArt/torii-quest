...
import {
  isHeartbeatDue, isFirstPublishDue, nextHeartbeatInMs, heartbeatStatus, isHeartbeatBroadcasting, HEARTBEAT_INTERVAL_MS, FIRST_PUBLISH_RETRY_MS,
} from './heartbeat.js';
...
describe('isFirstPublishDue (ADR-0077 auto-on + failed-first-publish backoff)', () => {
  it('is due now when never published and never attempted (fresh owner login)', () => {
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: null, now: 0 })).toBe(true);
    expect(isFirstPublishDue({ lastPublishedAt: undefined, lastAttemptedAt: undefined, now: 12345 })).toBe(true);
  });

  it('is NOT due once already successfully published (isHeartbeatDue takes over)', () => {
    // lastPublishedAt set → first publish is no longer "first"; republish cadence
    // is isHeartbeatDue's job, not this helper.
    expect(isFirstPublishDue({ lastPublishedAt: 0, lastAttemptedAt: 0, now: 0 })).toBe(false);
    expect(isFirstPublishDue({ lastPublishedAt: 1000, lastAttemptedAt: 1000, now: 999999 })).toBe(false);
  });

  it('is NOT due immediately after a failed first publish — backs off (no rAF spin)', () => {
    // Attempted 1ms ago, retry interval 30000 → not due until 30000.
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 1000, now: 1001, retryMs: 30000 })).toBe(false);
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 1000, now: 30999, retryMs: 30000 })).toBe(false);
  });

  it('is due again once the retry backoff has elapsed since the failed attempt', () => {
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 1000, now: 31000, retryMs: 30000 })).toBe(true);
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 1000, now: 60000, retryMs: 30000 })).toBe(true);
  });

  it('defaults retryMs to FIRST_PUBLISH_RETRY_MS (30000)', () => {
    expect(FIRST_PUBLISH_RETRY_MS).toBe(30000);
    // Attempted 29999ms ago → not due (default 30000).
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 0, now: 29999 })).toBe(false);
    // Attempted 30000ms ago → due.
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: 0, now: 30000 })).toBe(true);
  });

  it('is robust to non-numeric / missing inputs (fails closed to "due" only when never attempted)', () => {
    // Bad lastAttemptedAt is treated as "never attempted" → due now (safe: the
    // first publish is allowed to fire; a bad value can't block the beacon).
    expect(isFirstPublishDue({ lastPublishedAt: null, lastAttemptedAt: NaN, now: 0 })).toBe(true);
    expect(isFirstPublishDue({ lastPublishedAt: null, now: 0 })).toBe(true);
    // Bad lastPublishedAt is treated as null → still a first publish.
    expect(isFirstPublishDue({ lastPublishedAt: 'no', lastAttemptedAt: null, now: 0 })).toBe(true);
  });
});
