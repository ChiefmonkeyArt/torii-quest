...
export async function resolveWorldReference({ referenceEvent, fetchBlob, blossomServer } = {}) {
  ...
  // Trust boundary: the reference must be signed by its author (owner's npub),
  // otherwise a forged reference could point at arbitrary content under a real
  // npub's name. Fail-closed before touching any network.
...
export async function discoverWorldReference({ pubkeyHex, relays = [], relayReqFn } = {}) {
  ...
  const filter = buildWorldFilter({ authors: [pubkeyHex], limit: 24 });
