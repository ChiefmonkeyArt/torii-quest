...
// We publish at most once per pubkey per page load; the profile-refresh emit
// still updates the UI (renderGatewayCard/playerStats) but no longer re-signs.
...
async function publishOurWorldPresence() {
  ...
  if (_presencePublishedPubkey === pubkey) return; // already advertised this session
