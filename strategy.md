...
### 15-Hour Proof-of-Concept Route (ACTIVE — v0.2.139)
...
balloons stops at a green checkpoint and is logged in `todo.md`; track status in
`progress.md`. Deploy/publish remains a separate manual maintainer step.
...
4. **Leaderboard preview** — a minimal score/kill leaderboard sourced from
   ...
   build-only preview today (`leaderboardView.js`, v0.2.136).
5. **torii.quest GitHub update-check** — architecture so a torii.quest instance can
   ...
   landed v0.2.138 (`engine/update/updateCheck.js`, `UPDATE_CHECK.md`); the
   read-only releases fetch + the in-world prompt mesh are deferred host steps.
