...
const goodFiles = () => ({
  'torii-quest-todo.md': `# TODO\nCurrent version: ${V}\n- do things`,
  'torii-quest-progress.md': `# Progress\nCurrent version: ${V}\n`,
  'torii-quest-handoff.md': `# Handoff\n- Current version: ${V}\n`,
...
describe('checkDocConsistency', () => {
  ...
  it('HARD FAILS when a continuity doc has drifted off the current version', () => {
    ...
    files['torii-quest-progress.md'] = '# Progress\nCurrent version: v0.2.150-alpha\n';
    const r = checkDocConsistency({ version: V, files });
    expect(r.ok).toBe(false);
    expect(r.errors.some((e) => e.includes('torii-quest-progress.md') && e.includes(V))).toBe(true);
  });

  it('HARD FAILS when a core continuity doc is missing', () => {
    const files = goodFiles();
    delete files['torii-quest-handoff.md'];
    const r = checkDocConsistency({ version: V, files });
    expect(r.ok).toBe(false);
    expect(r.errors.some((e) => e.includes('missing core doc: torii-quest-handoff.md'))).toBe(true);
  });

  it('respects an explicit present:false over a supplied file body', () => {
    const r = checkDocConsistency({ version: V, files: goodFiles(), present: { 'torii-quest-todo.md': false } });
    expect(r.ok).toBe(false);
    expect(r.errors.some((e) => e.includes('missing core doc: torii-quest-todo.md'))).toBe(true);
  });
