...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.753-alpha'));
    ...
    expect(a.behindBy).toBe(473);  // 753-280=473 (tracks app version)
