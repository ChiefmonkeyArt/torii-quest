...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.750-alpha'));
    ...
    expect(a.behindBy).toBe(470);  // 750-280=470 (tracks app version)
