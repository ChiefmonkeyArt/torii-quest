...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.829-alpha'));
    ...
    expect(a.behindBy).toBe(549);  // 829-280=549 (tracks app version)
