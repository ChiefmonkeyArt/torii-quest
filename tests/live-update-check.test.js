...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.716-alpha'));
    ...
    expect(a.behindBy).toBe(436);  // 716-280=436 (tracks app version)
