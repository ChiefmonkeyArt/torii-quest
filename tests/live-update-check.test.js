...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.784-alpha'));
    ...
    expect(a.behindBy).toBe(504);  // 784-280=504 (tracks app version)
