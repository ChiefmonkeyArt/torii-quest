...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.764-alpha'));
    ...
    expect(a.behindBy).toBe(484);  // 764-280=484 (tracks app version)
