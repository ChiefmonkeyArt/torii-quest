...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.774-alpha'));
    ...
    expect(a.behindBy).toBe(494);  // 774-280=494 (tracks app version)
