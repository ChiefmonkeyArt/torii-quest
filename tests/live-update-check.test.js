...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.811-alpha'));
    ...
    expect(a.behindBy).toBe(531);  // 811-280=531 (tracks app version)
