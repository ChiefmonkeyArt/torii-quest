...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.883-alpha'));
    ...
    expect(a.behindBy).toBe(603);  // 883-280=603 (tracks app version)
