...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.892-alpha'));
    ...
    expect(a.behindBy).toBe(612);  // 892-280=612 (tracks app version)
