...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.777-alpha'));
    ...
    expect(a.behindBy).toBe(497);  // 777-280=497 (tracks app version)
