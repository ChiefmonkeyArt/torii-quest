...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.818-alpha'));
    ...
    expect(a.behindBy).toBe(538);  // 818-280=538 (tracks app version)
