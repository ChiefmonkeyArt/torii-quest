...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.843-alpha'));
    ...
    expect(a.behindBy).toBe(563);  // 843-280=563 (tracks app version)
