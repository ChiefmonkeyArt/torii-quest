...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.739-alpha'));
    ...
    expect(a.behindBy).toBe(459);  // 739-280=459 (tracks app version)
