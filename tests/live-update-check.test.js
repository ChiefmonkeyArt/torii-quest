...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.783-alpha'));
    ...
    expect(a.behindBy).toBe(503);  // 783-280=503 (tracks app version)
