...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.853-alpha'));
    ...
    expect(a.behindBy).toBe(573);  // 853-280=573 (tracks app version)
