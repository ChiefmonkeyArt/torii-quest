...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.322-alpha'));
    ...
    expect(a.behindBy).toBe(42);  // 322-280=42 (tracks app version)
