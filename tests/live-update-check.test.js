...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.728-alpha'));
    ...
    expect(a.behindBy).toBe(448);  // 728-280=448 (tracks app version)
