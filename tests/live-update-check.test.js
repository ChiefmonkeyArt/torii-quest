...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.734-alpha'));
    ...
    expect(a.behindBy).toBe(454);  // 734-280=454 (tracks app version)
