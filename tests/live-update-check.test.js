...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.722-alpha'));
    ...
    expect(a.behindBy).toBe(442);  // 722-280=442 (tracks app version)
