...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.749-alpha'));
    ...
    expect(a.behindBy).toBe(469);  // 749-280=469 (tracks app version)
