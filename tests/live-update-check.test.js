...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.845-alpha'));
    ...
    expect(a.behindBy).toBe(565);  // 845-280=565 (tracks app version)
