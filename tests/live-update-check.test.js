...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.868-alpha'));
    ...
    expect(a.behindBy).toBe(588);  // 868-280=588 (tracks app version)
