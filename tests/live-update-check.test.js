...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.761-alpha'));
    ...
    expect(a.behindBy).toBe(481);  // 761-280=481 (tracks app version)
