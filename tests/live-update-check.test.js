...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.773-alpha'));
    ...
    expect(a.behindBy).toBe(493);  // 773-280=493 (tracks app version)
