...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.817-alpha'));
    ...
    expect(a.behindBy).toBe(537);  // 817-280=537 (tracks app version)
