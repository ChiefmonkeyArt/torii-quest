...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.880-alpha'));
    ...
    expect(a.behindBy).toBe(600);  // 880-280=600 (tracks app version)
