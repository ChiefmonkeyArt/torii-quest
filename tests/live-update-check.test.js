...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.828-alpha'));
    ...
    expect(a.behindBy).toBe(548);  // 828-280=548 (tracks app version)
