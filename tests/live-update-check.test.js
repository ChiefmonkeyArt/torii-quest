...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.849-alpha'));
    ...
    expect(a.behindBy).toBe(569);  // 849-280=569 (tracks app version)
