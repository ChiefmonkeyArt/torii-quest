...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.796-alpha'));
    ...
    expect(a.behindBy).toBe(516);  // 796-280=516 (tracks app version)
