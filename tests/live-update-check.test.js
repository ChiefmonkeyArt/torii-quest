...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.875-alpha'));
    ...
    expect(a.behindBy).toBe(595);  // 875-280=595 (tracks app version)
