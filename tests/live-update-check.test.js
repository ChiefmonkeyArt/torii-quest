...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.717-alpha'));
    ...
    expect(a.behindBy).toBe(437);  // 717-280=437 (tracks app version)
