...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.886-alpha'));
    ...
    expect(a.behindBy).toBe(606);  // 886-280=606 (tracks app version)
