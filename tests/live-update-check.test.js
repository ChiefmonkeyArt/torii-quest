...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.882-alpha'));
    ...
    expect(a.behindBy).toBe(602);  // 882-280=602 (tracks app version)
