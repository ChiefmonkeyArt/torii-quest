...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.781-alpha'));
    ...
    expect(a.behindBy).toBe(501);  // 781-280=501 (tracks app version)
