...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.889-alpha'));
    ...
    expect(a.behindBy).toBe(609);  // 889-280=609 (tracks app version)
