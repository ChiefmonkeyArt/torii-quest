...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.731-alpha'));
    ...
    expect(a.behindBy).toBe(451);  // 731-280=451 (tracks app version)
