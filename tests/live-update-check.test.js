...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.797-alpha'));
    ...
    expect(a.behindBy).toBe(517);  // 797-280=517 (tracks app version)
