...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.779-alpha'));
    ...
    expect(a.behindBy).toBe(499);  // 779-280=499 (tracks app version)
