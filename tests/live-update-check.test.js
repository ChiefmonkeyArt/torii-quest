...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.775-alpha'));
    ...
    expect(a.behindBy).toBe(495);  // 775-280=495 (tracks app version)
