...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.885-alpha'));
    ...
    expect(a.behindBy).toBe(605);  // 885-280=605 (tracks app version)
