...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.823-alpha'));
    ...
    expect(a.behindBy).toBe(543);  // 823-280=543 (tracks app version)
