...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.874-alpha'));
    ...
    expect(a.behindBy).toBe(594);  // 874-280=594 (tracks app version)
