...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.846-alpha'));
    ...
    expect(a.behindBy).toBe(566);  // 846-280=566 (tracks app version)
