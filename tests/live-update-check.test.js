...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.299-alpha'));
    ...
    expect(a.behindBy).toBe(19);  // 299-280=19 (tracks app version)
