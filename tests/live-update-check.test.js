...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.757-alpha'));
    ...
    expect(a.behindBy).toBe(477);  // 757-280=477 (tracks app version)
