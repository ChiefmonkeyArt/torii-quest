...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.825-alpha'));
    ...
    expect(a.behindBy).toBe(545);  // 825-280=545 (tracks app version)
