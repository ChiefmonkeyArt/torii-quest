...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.738-alpha'));
    ...
    expect(a.behindBy).toBe(458);  // 738-280=458 (tracks app version)
