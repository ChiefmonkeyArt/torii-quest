...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.346-alpha'));
    ...
    expect(a.behindBy).toBe(66);  // 346-280=66 (tracks app version)
