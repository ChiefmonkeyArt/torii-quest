...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.813-alpha'));
    ...
    expect(a.behindBy).toBe(533);  // 813-280=533 (tracks app version)
