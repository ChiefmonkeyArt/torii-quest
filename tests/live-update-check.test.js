...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.867-alpha'));
    ...
    expect(a.behindBy).toBe(587);  // 867-280=587 (tracks app version)
