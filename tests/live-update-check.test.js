...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.827-alpha'));
    ...
    expect(a.behindBy).toBe(547);  // 827-280=547 (tracks app version)
