...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.786-alpha'));
    ...
    expect(a.behindBy).toBe(506);  // 786-280=506 (tracks app version)
