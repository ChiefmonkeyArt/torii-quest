...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.711-alpha'));
    ...
    expect(a.behindBy).toBe(431);  // 711-280=431 (tracks app version)
