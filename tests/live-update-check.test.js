...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.746-alpha'));
    ...
    expect(a.behindBy).toBe(466);  // 746-280=466 (tracks app version)
