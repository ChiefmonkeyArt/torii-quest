...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.724-alpha'));
    ...
    expect(a.behindBy).toBe(444);  // 724-280=444 (tracks app version)
