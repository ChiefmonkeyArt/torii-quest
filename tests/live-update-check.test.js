...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.834-alpha'));
    ...
    expect(a.behindBy).toBe(554);  // 834-280=554 (tracks app version)
