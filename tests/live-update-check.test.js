...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.832-alpha'));
    ...
    expect(a.behindBy).toBe(552);  // 832-280=552 (tracks app version)
