...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.351-alpha'));
    ...
    expect(a.behindBy).toBe(71);  // 350-280=70 (tracks app version)
