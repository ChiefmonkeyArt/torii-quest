...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.727-alpha'));
    ...
    expect(a.behindBy).toBe(447);  // 727-280=447 (tracks app version)
