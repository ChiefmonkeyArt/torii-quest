...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.891-alpha'));
    ...
    expect(a.behindBy).toBe(611);  // 891-280=611 (tracks app version)
