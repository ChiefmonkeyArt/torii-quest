...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.820-alpha'));
    ...
    expect(a.behindBy).toBe(540);  // 820-280=540 (tracks app version)
