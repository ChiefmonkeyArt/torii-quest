...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.866-alpha'));
    ...
    expect(a.behindBy).toBe(586);  // 866-280=586 (tracks app version)
