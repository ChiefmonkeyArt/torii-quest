...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.780-alpha'));
    ...
    expect(a.behindBy).toBe(500);  // 780-280=500 (tracks app version)
