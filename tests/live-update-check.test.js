...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.294-alpha'));
    ...
    expect(a.behindBy).toBe(14);  // 294-280=14 (tracks app version)
