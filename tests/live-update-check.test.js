...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.715-alpha'));
    ...
    expect(a.behindBy).toBe(435);  // 715-280=435 (tracks app version)
