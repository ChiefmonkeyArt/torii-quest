...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.884-alpha'));
    ...
    expect(a.behindBy).toBe(604);  // 884-280=604 (tracks app version)
