...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.830-alpha'));
    ...
    expect(a.behindBy).toBe(550);  // 830-280=550 (tracks app version)
