...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.888-alpha'));
    ...
    expect(a.behindBy).toBe(608);  // 888-280=608 (tracks app version)
