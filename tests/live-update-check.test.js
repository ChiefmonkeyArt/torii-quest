...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.748-alpha'));
    ...
    expect(a.behindBy).toBe(468);  // 748-280=468 (tracks app version)
