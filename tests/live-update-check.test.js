...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.803-alpha'));
    ...
    expect(a.behindBy).toBe(523);  // 803-280=523 (tracks app version)
