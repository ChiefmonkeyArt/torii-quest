...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.894-alpha'));
    ...
    expect(a.behindBy).toBe(614);  // 894-280=614 (tracks app version)
