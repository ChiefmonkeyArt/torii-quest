...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.856-alpha'));
    ...
    expect(a.behindBy).toBe(576);  // 856-280=576 (tracks app version)
