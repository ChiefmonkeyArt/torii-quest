...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.348-alpha'));
    ...
    expect(a.behindBy).toBe(68);  // 348-280=68 (tracks app version)
