...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.824-alpha'));
    ...
    expect(a.behindBy).toBe(544);  // 824-280=544 (tracks app version)
