...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.745-alpha'));
    ...
    expect(a.behindBy).toBe(465);  // 745-280=465 (tracks app version)
