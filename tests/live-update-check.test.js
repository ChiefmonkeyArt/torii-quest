...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.729-alpha'));
    ...
    expect(a.behindBy).toBe(449);  // 729-280=449 (tracks app version)
