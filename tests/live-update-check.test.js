...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.735-alpha'));
    ...
    expect(a.behindBy).toBe(455);  // 735-280=455 (tracks app version)
