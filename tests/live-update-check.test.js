...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.862-alpha'));
    ...
    expect(a.behindBy).toBe(582);  // 862-280=582 (tracks app version)
