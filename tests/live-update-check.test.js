...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.733-alpha'));
    ...
    expect(a.behindBy).toBe(453);  // 733-280=453 (tracks app version)
