...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.782-alpha'));
    ...
    expect(a.behindBy).toBe(502);  // 782-280=502 (tracks app version)
