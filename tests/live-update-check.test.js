...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.794-alpha'));
    ...
    expect(a.behindBy).toBe(514);  // 794-280=514 (tracks app version)
