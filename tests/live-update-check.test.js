...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.857-alpha'));
    ...
    expect(a.behindBy).toBe(577);  // 857-280=577 (tracks app version)
