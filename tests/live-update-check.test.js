...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.838-alpha'));
    ...
    expect(a.behindBy).toBe(558);  // 838-280=558 (tracks app version)
