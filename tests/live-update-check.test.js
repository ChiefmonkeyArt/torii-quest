...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.338-alpha'));
    ...
    expect(a.behindBy).toBe(58);  // 338-280=58 (tracks app version)
