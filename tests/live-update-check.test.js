...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.850-alpha'));
    ...
    expect(a.behindBy).toBe(570);  // 850-280=570 (tracks app version)
