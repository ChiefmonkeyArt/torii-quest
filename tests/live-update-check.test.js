...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.852-alpha'));
    ...
    expect(a.behindBy).toBe(572);  // 852-280=572 (tracks app version)
