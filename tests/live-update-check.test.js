...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.801-alpha'));
    ...
    expect(a.behindBy).toBe(521);  // 801-280=521 (tracks app version)
