...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.876-alpha'));
    ...
    expect(a.behindBy).toBe(596);  // 876-280=596 (tracks app version)
