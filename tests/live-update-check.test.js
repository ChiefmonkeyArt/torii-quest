...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.858-alpha'));
    ...
    expect(a.behindBy).toBe(578);  // 858-280=578 (tracks app version)
