...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.859-alpha'));
    ...
    expect(a.behindBy).toBe(579);  // 859-280=579 (tracks app version)
