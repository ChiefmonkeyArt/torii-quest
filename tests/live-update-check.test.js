...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.864-alpha'));
    ...
    expect(a.behindBy).toBe(584);  // 864-280=584 (tracks app version)
