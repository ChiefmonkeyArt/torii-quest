...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.835-alpha'));
    ...
    expect(a.behindBy).toBe(555);  // 835-280=555 (tracks app version)
