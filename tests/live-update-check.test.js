...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.833-alpha'));
    ...
    expect(a.behindBy).toBe(553);  // 833-280=553 (tracks app version)
