...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.808-alpha'));
    ...
    expect(a.behindBy).toBe(528);  // 808-280=528 (tracks app version)
