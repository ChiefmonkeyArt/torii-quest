...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.870-alpha'));
    ...
    expect(a.behindBy).toBe(590);  // 870-280=590 (tracks app version)
