...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.815-alpha'));
    ...
    expect(a.behindBy).toBe(535);  // 815-280=535 (tracks app version)
