...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.887-alpha'));
    ...
    expect(a.behindBy).toBe(607);  // 887-280=607 (tracks app version)
