...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.842-alpha'));
    ...
    expect(a.behindBy).toBe(562);  // 842-280=562 (tracks app version)
