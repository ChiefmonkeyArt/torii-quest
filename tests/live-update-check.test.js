...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.841-alpha'));
    ...
    expect(a.behindBy).toBe(561);  // 841-280=561 (tracks app version)
