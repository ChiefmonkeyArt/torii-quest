...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.776-alpha'));
    ...
    expect(a.behindBy).toBe(496);  // 776-280=496 (tracks app version)
