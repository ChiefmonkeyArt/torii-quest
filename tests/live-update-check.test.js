...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.816-alpha'));
    ...
    expect(a.behindBy).toBe(536);  // 816-280=536 (tracks app version)
