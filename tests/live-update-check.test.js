...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.844-alpha'));
    ...
    expect(a.behindBy).toBe(564);  // 844-280=564 (tracks app version)
