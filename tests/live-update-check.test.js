...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.721-alpha'));
    ...
    expect(a.behindBy).toBe(441);  // 721-280=441 (tracks app version)
