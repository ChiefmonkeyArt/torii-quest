...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.873-alpha'));
    ...
    expect(a.behindBy).toBe(593);  // 873-280=593 (tracks app version)
