...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.869-alpha'));
    ...
    expect(a.behindBy).toBe(589);  // 869-280=589 (tracks app version)
