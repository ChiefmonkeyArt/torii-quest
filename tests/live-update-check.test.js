...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.848-alpha'));
    ...
    expect(a.behindBy).toBe(568);  // 848-280=568 (tracks app version)
