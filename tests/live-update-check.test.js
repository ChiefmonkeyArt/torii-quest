...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.837-alpha'));
    ...
    expect(a.behindBy).toBe(557);  // 837-280=557 (tracks app version)
