...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.752-alpha'));
    ...
    expect(a.behindBy).toBe(472);  // 752-280=472 (tracks app version)
