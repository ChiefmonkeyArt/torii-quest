...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.800-alpha'));
    ...
    expect(a.behindBy).toBe(520);  // 800-280=520 (tracks app version)
