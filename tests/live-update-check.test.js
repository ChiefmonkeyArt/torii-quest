...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.331-alpha'));
    ...
    expect(a.behindBy).toBe(51);  // 331-280=51 (tracks app version)
