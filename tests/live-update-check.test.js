...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.304-alpha'));
    ...
    expect(a.behindBy).toBe(24);  // 304-280=24 (tracks app version)
