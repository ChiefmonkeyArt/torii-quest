...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.863-alpha'));
    ...
    expect(a.behindBy).toBe(583);  // 863-280=583 (tracks app version)
