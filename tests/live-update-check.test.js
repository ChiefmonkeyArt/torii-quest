...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.759-alpha'));
    ...
    expect(a.behindBy).toBe(479);  // 759-280=479 (tracks app version)
