...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.726-alpha'));
    ...
    expect(a.behindBy).toBe(446);  // 726-280=446 (tracks app version)
