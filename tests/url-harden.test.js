...
describe('urlHarden — hardenSpawnUrl accept path', () => {
  it('accepts a clean https URL and re-emits the canonical href', () => {
    const r = hardenSpawnUrl('https://chiefmonkey.art');
    expect(r.ok).toBe(true);
    expect(r.errors).toEqual([]);
    expect(r.url).toBe('https://chiefmonkey.art/');
  });
  ...
  it('accepts an allowlisted host with a subdomain', () => {
    const r = hardenSpawnUrl('https://other-host.example.com/');
    expect(r.ok).toBe(true);
  });
});
...
describe('urlHarden — hardenSpawnUrl port enforcement (v0.2.260 S2)', () => {
  ...
  it('rejects an allowlisted host that uses a non-default port (host + port enforced independently)', () => {
    const r = hardenSpawnUrl('https://chiefmonkey.art:8443/', {
      allowHosts: ['chiefmonkey.art'],
    });
    expect(r.ok).toBe(false);
    expect(r.errors).toContain('non-default-port-rejected');
...
describe('urlHarden — appendTraveller', () => {
  ...
  it('appends the traveller pubkey as a query param', () => {
    const r = appendTraveller('https://chiefmonkey.art/', PK);
    expect(r.ok).toBe(true);
