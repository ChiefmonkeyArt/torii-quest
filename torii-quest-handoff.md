...
**Current version:** v0.2.574-alpha - PER-BONE NPC STICKER COLLIDERS. Prior: v0.2.573-alpha - LOAD TIME OPTIMIZATION (Draco + WebP texture compression, smooth progress bar, title-screen preloading). See §0 for the current snapshot.
...
- **v0.2.574-alpha - PER-BONE NPC STICKER COLLIDERS.** Added per-bone Rapier ball sensor colliders (radius 0.10) on all 24 NPC skeleton bones so stickers can hit individual body parts (arms, legs, torso, head) and follow bone animation. Uses two-raycast approach: one bone-only raycast (filtered by colliderToBone) and one broad raycast (excludes bone colliders) — prevents the broad capsule from shadowing small bone spheres. When a bone is hit, sticker is parented via Three.js Object3D.attach() which converts world transform to bone-local space — sticker follows bone animation automatically. Broad capsule kept as fallback for gaps between bone spheres. Bone colliders synced each frame after mixer.update() + updateMatrixWorld(). castRayStatic excludes bone colliders (bullets/LOS). Research-based approach matching Unreal FindClosestBone, Unity per-bone triggers, Three.js bone.attach(). NPC only this pass; mechanism is reusable for bots/peers. 2668 tests pass, 21 checks green.
...
- **Version:** v0.2.574-alpha (tag `v0.2.574-alpha`).
...
**PENDING DEPLOY:** v0.2.574-alpha is pushed to GitHub (tag `v0.2.574-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.574-alpha
```
