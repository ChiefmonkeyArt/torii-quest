...
**Current version:** v0.2.533-alpha - BRIDGE MOVE + BOT BRIDGE CROSSING + FLY MODE DAMAGE FIX. Prior: v0.2.532-alpha - TORII GATE 90° CW ROTATION. See §0 for the current snapshot.
...
- **v0.2.533-alpha - BRIDGE MOVE + BOT BRIDGE CROSSING + FLY MODE DAMAGE FIX.** (1) Moved Bridge 1 (NAP ↔ Arena BL) 10m south + 10m west from (-24, 3) to (-34, -7). Entrance torii gate moves with it (uses same constants). (2) Bots can now walk over Bridge 2 (Arena BL ↔ BR) between the two play-zone islands. Added `isBridgeWalkable` optional dep to `createBotSim` — when a bot is on Bridge 2, coastline clamping is skipped so it can walk freely. Added `bridgeWaypoints` for inter-island pathing: bots on a different island than the player steer toward the bridge entry point first. Bots NOT on a bridge are still clamped to their island (can't cross water/glow rings). Bridge 1 (NAP bridge) is NOT whitelisted for bots. (3) Fly mode damage fix: bot bullets now deal no damage when player is flying above FLY_TARGET_CEILING (21m). Hard gate in weapons.js bot-bullet collision mirrors the AI's tooHighToTarget check. Combat values, score publishing, and MP protocol untouched.
...
- **Version:** v0.2.533-alpha (tag `v0.2.533-alpha`).
...
**PENDING DEPLOY:** v0.2.533-alpha is pushed to GitHub (tag `v0.2.533-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.533-alpha
```
