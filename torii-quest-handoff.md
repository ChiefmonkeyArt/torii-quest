...
**Current version:** v0.2.537-alpha - TORII GATE ROTATED 315°. Prior: v0.2.536-alpha - BRIDGE ROTATION PIVOT FIX. See §0 for the current snapshot.
...
- **v0.2.537-alpha - TORII GATE ROTATED 315°.** Set entrance torii gate (both fallback and GLB) rotation.y to 7π/4 (315°). Also fixed inconsistent rotations — fallback was 90° while GLB was 180°. Both now match at 315°. Combat / score publishing untouched.
...
- **Version:** v0.2.537-alpha (tag `v0.2.537-alpha`).
...
**PENDING DEPLOY:** v0.2.537-alpha is pushed to GitHub (tag `v0.2.537-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.537-alpha
```
