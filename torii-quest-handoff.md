...
**Current version:** v0.2.370-alpha (source at v0.2.370-alpha; LIVE at v0.2.369-alpha on https://chiefmonkey.art/quest/ — FROZEN after ENTER ARENA under the `/quest/` mount; the v0.2.370 CSP base-aware freeze fix is source-only, awaiting a Suite redeploy. Torii Suite SHC VPS install; pplx.app backend sandbox still 503, kept as secondary. See §9 + “Deployment conventions” in torii-quest-todo.md)
...
- **v0.2.370-alpha — CSP-plugin freeze fixed under the `/quest/` mount (base-aware entry strip); LIVE still v0.2.369-alpha/FROZEN until a Suite redeploy.** v0.2.369-alpha at chiefmonkey.art/quest/ freezes in BOTH Firefox + Brave right after ENTER ARENA (menu responsive; reproduces anonymously). Root cause: `cspHeaderPlugin()` in `vite.config.js` hardcoded root-relative entry URLs. Its two strip regexes used the ESCAPED literal `\/assets\/torii-entry\.js` — the Torii Suite's install-time sed does NOT rewrite the escaped form, so under the `/quest/` mount they only matched the ROOT static tag, never the `/quest/`-prefixed one. The parser-inserted `<script src="/quest/assets/torii-entry.js">` therefore SURVIVED alongside the plugin's versioned `import('/quest/assets/torii-entry.js?v=…')` → the entry loaded under TWO URLs → two module instances → arenaRuntime (dynamic-imported on ENTER ARENA) bound to a DIFFERENT entry instance than the running menu/game state → arena boot hung → freeze (breaks the v0.2.285 single-entry-instance dedupe invariant). Fix: `cspHeaderPlugin()` is now natively base-aware — a `configResolved(config)` hook captures `config.base` (trailing-slash normalised) → `ENTRY_URL = ${base}assets/torii-entry.js`; the inline import + chunk back-ref rewrite both emit `${ENTRY_URL}?v=${BUILD_STAMP}`, and the two strip regexes are now base-AGNOSTIC (`src="[^"]*assets/torii-entry\.js"` / `href="…"`), deliberately written WITHOUT the literal `/assets/torii-entry.js` substring so the Suite sed can't corrupt them → the static tag is stripped under ANY base. `tools/csp.mjs` untouched (no CSP header is served on chiefmonkey.art — the freeze is purely the double instance, not a CSP violation). Root base (`/`) behaviour is byte-identical to before. Verified: build green, check 20/20, tests 2264/2264; a definitive `--base=/quest/` build confirms NO surviving static `<script src=…torii-entry.js>` tag, exactly ONE inline `import('/quest/assets/torii-entry.js?v=…')`, and the arenaRuntime chunk back-ref `/quest/assets/torii-entry.js?v=…` (same stamp) → single entry instance. **Source-only — the Suite installer is pinned at `TORII_QUEST_REF=v0.2.369-alpha` (we just bumped torii-suite#24); the maintainer must bump the Suite Quest ref to v0.2.370-alpha (or the merge commit) before a Quest-only redeploy (`install-quest.sh`, builds `--base=/quest/`) will pick up the fix; do NOT touch the VPS — shared host, Continuum session paused.** The freeze is not cleared live until a real-browser retest on the redeployed v0.2.370 build shows ENTER ARENA booting into the arena.
