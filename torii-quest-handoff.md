...
**Current version:** v0.2.611-alpha - HOMEPAGE SIMPLIFY + ARENA SHOOTER LEADERBOARD + NPC CLEANUP (homepage stripped to a single ENTER; Gateway Setup moved into Admin Settings + Torii menu; 3D homescreen fixed — Draco decoder wired, software-GL gate removed, chiefmonkey armature-scale fix, fog/palette lift; in-game leaderboard titled ARENA SHOOTER with live consent-gated Nostr + gamestr.io publishing under game id `arena-shooter`; Augustink nameplate removed + boss HP bar shrunk to a chest readout; NPC gestures filtered to in-place clips only; duplicate gun SFX/recoil fixed). Prior: v0.2.610-alpha - GAMEPLAY FOLLOW-UPS (ESC pause recency-gate; MP bridge routing parity + deck-crossing fix; MP invisible-bot self-heal + corpse cleanup). Prior: v0.2.609-alpha - 3D HOMEPAGE SCENE (starfield + torii gate + slow camera orbit behind the home-surface card; lazy `import('three')`, single rAF, full dispose on unmount; fail-safe to the DOM gradient when WebGL is absent; no fetch/sign/navigate in the homepage layer). Prior: v0.2.606-alpha - COMPONENT SEAM (0l.1 expand path + crates-as-components, 0l.2 runtime host + beacon, 0l.3 portal/NAP data-shell). Prior: v0.2.602-alpha - PER-BONE STICKER COLLIDERS ON BOTS. Prior: v0.2.574-alpha - PER-BONE NPC STICKER COLLIDERS. See §0 for the current snapshot.

**Recent shipped work (newest first):**
- **v0.2.610-alpha - GAMEPLAY FOLLOW-UPS (ESC pause + MP bots).** (1) ESC PAUSE UNPROMPTED: the bare-keyup fallback (for the browser-reserved pointer-lock ESC) fired `_openPause()` on ANY ESC keyup while playing unlocked — e.g. dismissing a NIP-07 signer prompt or the browser find bar. New `wasLockReleasedRecently()` in `src/input.js` gates the keyup fallback to within 1.5s of a real pointer-lock release. (2) BOTS NOT HONING (MP), two-layer root cause: `server/bots/arenaBotSim.js` never received `isBridgeWalkable`/`bridgeWaypoints` (cross-island targets beached bots at the coastline clamp), AND the shared brain dropped the bridge waypoint the tick a bot stepped onto the deck (`whichIsland(deck)===ISLAND_NONE` failed the island-only condition — would have beached SP crossings too). New shared pure module `src/engine/entities/bridge2Walk.js` (`isOnBridge2` + `BRIDGE2_WAYPOINTS` from config) wired into BOTH sims; on-deck bots route to the far-side waypoint until they land on the target's island. Regression: `tests/multiplayer/arena-bot-bridge.test.js` (4) with a live-tick cross-island pursuit. (3) INVISIBLE BOTS (MP): `_ensureNetCapsule()` self-heal re-materialises a capsule placeholder when a server-alive wrapper has neither model nor capsule; MP corpses now hide at t>1.4s after the death launch-arc lands (previously ran forever) and stop paying the mixer tick. Deploy: `sudo torii-deploy v0.2.610-alpha`.
- **v0.2.608-alpha - 3D HOMEPAGE SCENE.** New `src/engine/homepage/homepageScene.js`: a self-contained Three.js scene (starfield `Points` + a torii-gate `Group` of cylinders/boxes + dark ground disc + fog + ambient/point/rim lights) rendered behind the home-surface card. Lazy `import('three')` inside `mount()` (cached module var), creates its own `<canvas>` inside a scene-host div, sizes via `ResizeObserver`, runs a single `rAF` loop (slow camera orbit + starfield counter-rotation + accent-emissive pulse). `unmount()` cancels the rAF, disconnects the `ResizeObserver`, disposes the renderer + every geometry/material, + removes the canvas — no orphaned GL context. Fail-safe: missing three / no WebGL (probed with a throwaway canvas + `failIfMajorPerformanceCaveat`, separate from the real render canvas so Three never receives a poisoned context) / any throw → returns `null` → the DOM gradient remains. Wired into `homepageStub.js`: a click-through scene-host div (`#torii-homepage-stub-scene`) sits behind the now-glassy card; `_mountScene` is async fire-and-forget with a `_hasWebGL()` pre-import gate so the three chunk is never requested in jsdom tests; `_close()` unmounts; `_resetForTest()` resets the new state. 2906 tests pass, regression check ALL GREEN. Deploy: `sudo torii-deploy v0.2.608-alpha`.
...
Deploy target is the pplx.app subdomain `chiefmonkey.art/quest`. Build artifact is `dist/` (`npm run build`). **Maintainer shortcut: "bump and push" = bump version, update the live site chiefmonkey.art/quest, and push everything to GitHub.**
