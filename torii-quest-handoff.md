...
**Current version:** v0.2.531-alpha - TORII GATE YAW ZEROED (gate flush with east-west boundary, text faces south into NAP zone). Prior: v0.2.530-alpha - TORII GATE ROTATION FLIP. See §0 for the current snapshot.
...
- **v0.2.531-alpha - TORII GATE YAW ZEROED.** Set TRAVEL_GATE_YAW_DELTA = 0 (was +55° CCW). The gate now sits flush with the east-west boundary at (0, 32) — no angle, no straddling. The GLB base yaw (Math.PI) was originally calibrated so the text "Torii Gateway Experience" faces south toward the approaching player; the yaw delta was causing it to sit at an angle across the boundary. Combat / score publishing untouched.
...
- **Version:** v0.2.531-alpha (tag `v0.2.531-alpha`).
...
| Torii gate (travel gateway) | (0, 32) | `Math.PI` (delta=0, flush east-west, text faces south) | `src/config.js` (TRAVEL_GATE_*) + `src/arena.js` |
...
**Key values:**
- `TRAVEL_GATE_X = 0`, `TRAVEL_GATE_Z = 32`, `TRAVEL_GATE_YAW_DELTA = 0` (flush east-west, text faces south) (in `src/config.js`)
...
**PENDING DEPLOY:** v0.2.531-alpha is pushed to GitHub (tag `v0.2.531-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.531-alpha
```
