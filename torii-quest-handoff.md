...
| `src/engine/dashboard/toriiQuestData.js` | `TORII_QUEST_VERSION` — pinned to `config.js` `VERSION` by `torii-quest-dashboard.test.js`; also the `metrics` "Source version" + "Tests" rows (test count) |
...
- **Task queue split (v0.2.244+):** the active task queue is now split. **`torii-quest-todo.md`**
  ...
  the Torii Quest (oversight dashboard app) active task source of truth. **`torii-quest-todo.md`**
...
- **`src/sdk/index.js`** — public SDK entrypoint (ARS-5). Curated node-safe
  ...
  **Hard-refresh deep-link resolution is NOT solvable in app code alone — it needs a
  ...
  `tools/build-torii-quest-dashboard.mjs` reads the required docs + walks `dist/` at packaging time
  (absent → dist check SKIPPED — `build:torii-quest-dashboard` runs before `vite build`, so
  regression-check [15] stays the authoritative dist check) and feeds the real verdict in.
  `toriiQuestDataJSON` now carries `readiness`. Server-rendered escaped text, NO new
  ...
  gameplay change. Unit-tested by `tests/torii-quest-dashboard.test.js`);
  ...
  is unchanged via a `realpathSync` run-guard), and `tools/build-torii-quest-dashboard.mjs` folds its
  result through `buildShipModel` at packaging time (degrading to last-known on any error).
  `toriiQuestDataJSON` now carries `ship`. Server-rendered escaped text, NO new `<script>`/
  ...
  `tests/torii-quest-dashboard.test.js` (+10).
  ...
  **v0.2.194** added a NOSTR READ-PATH HEALTH model — a pure, node-safe read-only health model
  ...
  `readHealthReport()` in `shellReport.js`), and a new **Nostr read-path health** Torii Quest
  panel (`buildReadHealthModel` in `toriiQuestData.js` maps each signal onto the existing pill
  vocabulary → the torii-quest CSP/script-hash are untouched). `tests/nostr-read-health.test.js`
  ...
  **v0.2.200** added a DASHBOARD METRIC FRESHNESS CLEANUP — resolves the recurring stale LAST-KNOWN
  ...
  "Total tests" metric and the metrics "Tests" row had diverged. `engine/dashboard/toriiQuestData.js`
  ...
  so the two displayed surfaces can no longer drift apart. A new `tests/torii-quest-dashboard.test.js`
  ...
  **v0.2.243** ZONE RENDERABLE-ROUTE REPAIR (game slice) — fixes the v0.2.242 follow-up: a live fetch saw the
  ...
  `build-torii-quest-dashboard.mjs` are all reconciled to the directory-index `/zone/<slug>/index.html` shell. Updated
  ...
  **v0.2.242** ZONE DEEP-LINK EXACT-PATH REPAIR (game slice) — fixes the live v0.2.241 smoke failure: a direct
  ...
  (`zoneFallbackReadiness`, `npm run zones:check`, regression `[15]`), and `build-torii-quest-dashboard.mjs` are all reconciled
  ...
  **v0.2.237** WORKFLOW-INVARIANT SLICE (status/dashboard/docs-only, no runtime change) — records a standing
  ...
  `tests/handoff-control-panel.test.js`, `tests/torii-quest-dashboard.test.js`, `tests/next-action-state.test.js`.
  ...
  **NON-RELIGIOUS** operating principles (self-sovereignty, consent, privacy by default, user-owned npub,
  ...
  consumed by BOTH the page (`toriiQuestData.js`) and the next-action state (`tools/nextActionState.mjs` +
  CLI + `tools/build-torii-quest-dashboard.mjs`), so the "is the handoff surface complete?" logic can never drift
  ...
  **false** everywhere. The two former at-a-glance "Live smoke" / "Dashboard smoke" rows are consolidated
  ...
  `tests/torii-quest-dashboard.test.js` and `tests/next-action-state.test.js` (suite 1548/94 → 1574/95).
  ...
  **v0.2.223** MVP PLAYTEST RESULTS ON THE CONTINUUM DASHBOARD — a docs/dashboard/tooling slice (no
  ...
  New PURE, browser-safe `buildPlaytestResultsCardModel(input={})` in `toriiQuestData.js` (badge
  ...
  **Playtest results** card placed BETWEEN the MVP-approval card and the Manual-validation card: it
  ...
  counts, any failing item ids, and a clear next step. `tools/build-torii-quest-dashboard.mjs` reads
  `MVP_PLAYTEST_RESULTS.md` (read-only `stat`+read), re-shapes it through the pure
  `summarizePlaytestForState` model, and folds it via `buildToriiQuestModel({playtestResults})`; any
  failure degrades to the curated not-run card (no toriiQuestData→tools import). `approvalImplied` is
  ...
  (pills `manual`/`open-edge`/`no-blocker`) so the torii-quest CSP + inline refresh-script hash are
  untouched; every value HTML-escaped. `tests/torii-quest-dashboard.test.js` (+8; suite 1463/89 →
  1471/89): last-known not-run, live not-run with next step, attention with failing ids, complete
  NOT-AN-APPROVAL with `approvalImplied:false`, pill vocabulary, `toriiQuestDataJSON` carries
  ...
  **v0.2.221** MVP APPROVAL DASHBOARD CARD — a docs/dashboard/tooling slice (no runtime change)
  ...
  `toriiQuestData.js` (badge `MVPAPPROVAL_BADGE` + frozen `MVPAPPROVAL_LASTKNOWN` fallback) renders a
  ...
  `tools/build-torii-quest-dashboard.mjs` from `MVP_APPROVAL_STATE.json` via `summarizeApprovalForState` and
  injected as plain data, keeping `toriiQuestData.js` free of any `tools/` import. +7
  `tests/torii-quest-dashboard.test.js` cases (model defaults/strictness, JSON carry, render + escaping)
  ...
  **v0.2.217** MACHINE-READABLE NEXT-ACTION STATE — a dashboard/docs/tooling slice (no runtime
  ...
  `buildManualValidationModel(...)` pill (`pill !== 'no-blocker'`) `build-torii-quest-dashboard.mjs` already uses,
  ...
  **v0.2.216** CONTINUUM NO-BLOCKER QUEUE CARD — a dashboard/docs/tooling slice (no runtime change).
  The Torii Quest oversight dashboard now surfaces a read-only **No-blocker queue** section just
  ...
  `buildNoBlockerQueueModel(input?)` in `src/engine/dashboard/toriiQuestData.js` DERIVES the counts
  from the SAME `taskTotals` already parsed off torii-quest-todo.md/torii-quest-progress.md by `tools/toriiQuestDashboardParse.mjs`
  (`activeNow`/`next12`/`archiveClusters`/`completed24h`/`todoCompletedMarkers`) — no second source of
  truth — folded in via `buildToriiQuestModel({noBlockerQueue})` with a `manualPending` flag read off
  ...
  torii-quest CSP + inline refresh-script hash are untouched; every value HTML-escaped.
  `tests/torii-quest-dashboard.test.js` (+8; suite now 1404/86): last-known model shape, live generated
  band separating safe vs user-gated, manual-clear variant, invalid/omitted counts fall back, pill
  vocabulary, `toriiQuestDataJSON` carries `noBlockerQueue`, render shows the section, hostile-input
  ...
  **v0.2.215** CONTINUUM MANUAL-VALIDATION / MVP-PLAYTEST READINESS CARD — a dashboard/docs/tooling
  slice (no runtime change). The Torii Quest oversight dashboard now surfaces a read-only
  **Manual validation** section just below the RC / release-manifest card that CLEARLY SEPARATES what
  ...
  `buildManualValidationModel(input?)` in `src/engine/dashboard/toriiQuestData.js` DERIVES this from
  ...
  only — no crypto/git/network) by `tools/build-torii-quest-dashboard.mjs` and folded in via
  `buildToriiQuestModel({manualValidation})` — rather than duplicating gate logic; with no input it
  ...
  the existing `.metric`/`.pill` markup so the torii-quest CSP + inline refresh-script hash are
  untouched; every value HTML-escaped. `tests/torii-quest-dashboard.test.js` (+8; suite now 1396/86):
  last-known model, live generated band separating automated vs manual, PLAYTEST DOCS INCOMPLETE on a
  missing doc, MANUAL VALIDATION OUTSTANDING on a non-green gate, pill vocabulary, `toriiQuestDataJSON`
  ...
  **v0.2.214** CONTINUUM RC / RELEASE-MANIFEST STATUS CARD — a dashboard/docs/tooling slice (no
  runtime change). The Torii Quest oversight dashboard now surfaces a read-only **RC / release
  ...
  `buildRcStatusModel(input?)` in `src/engine/dashboard/toriiQuestData.js` DERIVES this from existing
  ...
  only — no crypto/git/network) by `tools/build-torii-quest-dashboard.mjs` and folded in via
  `buildToriiQuestModel({rcStatus})` — rather than duplicating gate logic; with no input it degrades to
  ...
  so the torii-quest CSP + inline refresh-script hash are untouched; every value HTML-escaped.
  `tests/torii-quest-dashboard.test.js` (+7; suite now 1388/86): last-known model, live generated band,
  ARTIFACTS INCOMPLETE on a missing required artifact/RC doc, pill vocabulary, `toriiQuestDataJSON`
  ...
  **v0.2.208** PROGRESS PARSER GAP CLEANUP — a pure build-time parser/doc fix (no runtime change).
  ...
  defaults. Root cause was the per-section bounds in `tools/toriiQuestDashboardParse.mjs` `deriveToriiQuestData()`,
  ...
  `npm run build:torii-quest-dashboard` derives all four lists from torii-quest-progress.md (`next12 (12), activeNow (34),
  completed24h (26), archive (11)`; gaps: none). `tests/torii-quest-dashboard.parse.test.js` (+2; 15→17; suite now
  ...
  v0.2.171 added `continuum` (the Torii Quest project-oversight dashboard
  data model + pure static-page renderer — read-only, no live writes; v0.2.174
  added a `buildToriiQuestModel(overrides)` merge seam fed by the build-time doc
  parser `tools/toriiQuestDashboardParse.mjs`, so the page DERIVES its list sections from
...
npm run build    # production build → dist/ (runs build:torii-quest-dashboard first → public/continuum.html + torii-quest-data.json)
npm run build:torii-quest-dashboard  # (re)generate the Torii Quest dashboard page + packaged data from torii-quest-progress.md model
