...
**Current version:** v0.2.534-alpha - BRIDGE 1 FURTHER MOVE (another 10m south + 10m west to -44, -17). Prior: v0.2.533-alpha - BRIDGE MOVE + BOT BRIDGE CROSSING + FLY MODE DAMAGE FIX. See §0 for the current snapshot.
...
- **v0.2.534-alpha - BRIDGE 1 FURTHER MOVE.** Moved Bridge 1 (+ entrance torii gate) another 10m south + 10m west from (-34, -7) to (-44, -17). Total move from original: 20m south + 20m west. Both share BRIDGE_X/BRIDGE_Z constants so move together. Combat / score publishing untouched.
...
- **Version:** v0.2.534-alpha (tag `v0.2.534-alpha`).
...
**PENDING DEPLOY:** v0.2.534-alpha is pushed to GitHub (tag `v0.2.534-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.534-alpha
```
