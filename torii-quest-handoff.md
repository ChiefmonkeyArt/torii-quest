...
**Current version:** v0.2.530-alpha - TORII GATE ROTATION FLIP (east side now flush with curved NAP boundary instead of west side). Prior: v0.2.529-alpha - BOOT LOADING OVERLAY. See §0 for the current snapshot.
...
- **v0.2.530-alpha - TORII GATE ROTATION FLIP.** Flipped the NAP zone torii gateway yaw delta from -55° CW to +55° CCW so the RIGHT (east) side of the gate is flush with the curved NAP boundary instead of the left (west) side. TRAVEL_GATE_YAW_DELTA changed from `-Math.PI/4 - Math.PI/18` to `+Math.PI/4 + Math.PI/18`. Direction convention formally agreed: torii gateway is NORTH (+Z), left = west (-X), right = east (+X). Combat / score publishing untouched.
...
- **Version:** v0.2.530-alpha (tag `v0.2.530-alpha`).
...
| Torii gate (travel gateway) | (0, 32) | `Math.PI + PI/4 + PI/18` (+55° CCW, east side flush) | `src/config.js` (TRAVEL_GATE_*) + `src/arena.js` |
...
**Key values:**
- `TRAVEL_GATE_X = 0`, `TRAVEL_GATE_Z = 32`, `TRAVEL_GATE_YAW_DELTA = Math.PI / 4 + Math.PI / 18` (+55° CCW, east side flush) (in `src/config.js`)
...
**PENDING DEPLOY:** v0.2.530-alpha is pushed to GitHub (tag `v0.2.530-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.530-alpha
```
