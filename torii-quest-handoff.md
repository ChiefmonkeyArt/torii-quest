...
**Current version:** v0.2.535-alpha - BRIDGE 1 RESET TO ORIGINAL (-24, 3). Prior: v0.2.534-alpha - BRIDGE 1 FURTHER MOVE. See §0 for the current snapshot.
...
- **v0.2.535-alpha - BRIDGE 1 RESET.** Reset Bridge 1 (+ entrance torii gate) back to original position (-24, 3) after moves to (-34, -7) and (-44, -17) pushed it into open sea. Bridge and torii gate share BRIDGE_X/BRIDGE_Z constants and are at the same coordinates. Combat / score publishing untouched.
...
- **Version:** v0.2.535-alpha (tag `v0.2.535-alpha`).
...
**PENDING DEPLOY:** v0.2.535-alpha is pushed to GitHub (tag `v0.2.535-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.535-alpha
```
