...
**Current version:** v0.2.532-alpha - TORII GATE 90° CW ROTATION. Prior: v0.2.531-alpha - TORII GATE YAW ZEROED. See §0 for the current snapshot.
...
- **v0.2.532-alpha - TORII GATE 90° CW ROTATION.** Set TRAVEL_GATE_YAW_DELTA = -Math.PI/2 (90° clockwise from south-facing base). Gate plane now runs north-south. Combat / score publishing untouched.
...
- **Version:** v0.2.532-alpha (tag `v0.2.532-alpha`).
...
**PENDING DEPLOY:** v0.2.532-alpha is pushed to GitHub (tag `v0.2.532-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.532-alpha
```
