...
**Current version:** v0.2.540-alpha - REMOVED GROUND GLOW RIBBON (light-blue water planes). Prior: v0.2.539-alpha - TORII GATE ROTATED 45°. See §0 for the current snapshot.
...
- **v0.2.540-alpha - REMOVED GROUND GLOW RIBBON.** Removed the additive cyan ground-wash ribbon (`_groundGlowMat`) from `_buildCoastlineWall()` in arena.js. The ribbon was rendering as translucent light-blue planes on the water around the two arena play rings, visible as fake water pools. Removed both the mesh generation and the unused material. Neon top edge and glass wall remain. Combat / score publishing untouched.
...
- **Version:** v0.2.540-alpha (tag `v0.2.540-alpha`).
...
**PENDING DEPLOY:** v0.2.540-alpha is pushed to GitHub (tag `v0.2.540-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.540-alpha
```
