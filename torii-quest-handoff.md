...
- **Live deployment trails source** by several versions — needs manual smoke + publish (`TQ-MANUAL-113`). **2026-07-11:** static frontend is now live at v0.2.366-alpha but the backend sandbox is DOWN (`GET /port/5000/` → 503/timeout). Republish blocked by the pplx-tool bridge outage (diagnostic `9a174646`). See QA-MP-BLOCKER-1 in `torii-quest-todo.md`.
