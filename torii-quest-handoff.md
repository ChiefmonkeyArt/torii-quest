...
**Current version:** v0.2.536-alpha - BRIDGE ROTATION PIVOT FIX (bridge visual now matches torii gate position). Prior: v0.2.535-alpha - BRIDGE 1 RESET TO ORIGINAL. See §0 for the current snapshot.
...
- **v0.2.536-alpha - BRIDGE ROTATION PIVOT FIX.** Fixed a long-standing bug in bridge.js `_buildOne`: the deck mesh was positioned at (x, y, z) inside a group at origin (0,0,0), so when the group was rotated 45° (BRIDGE_YAW), the deck swung around the world origin instead of its own center. The visual bridge was ~19m away from its intended position (-24, 3), while the physics collider and torii gate were correctly at (-24, 3). Fix: set `group.position.set(x, 0, z)` and position the deck/rails at origin within the group, so rotation pivots at (x, z). Now the visual bridge, physics collider, and torii gate are all at the same coordinates. Bridge 2 (no rotation) is unaffected. Combat / score publishing untouched.
...
- **Version:** v0.2.536-alpha (tag `v0.2.536-alpha`).
...
**PENDING DEPLOY:** v0.2.536-alpha is pushed to GitHub (tag `v0.2.536-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.536-alpha
```
