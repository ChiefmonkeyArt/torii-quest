...
**Current version:** v0.2.372-alpha (source at v0.2.372-alpha; LIVE at v0.2.370-alpha on https://chiefmonkey.art/quest/ — the v0.2.370 CSP base-aware fix deployed, so anonymous ENTER ARENA now BOOTS into the arena. Remaining open edges, both fixed in this v0.2.372 source pending a Suite redeploy: (1) players cannot see each other (H4) — peer-avatar GLTFLoader was missing a DRACOLoader (fixed v0.2.371); (2) repeating NIP-07 signer-prompt loop on the pause/idle screen — the client sent no keepalive, so the server idle-dropped silent sessions at 60s → reconnect → re-auth loop (fixed v0.2.372, client keepalive PING every 25s). Torii Suite SHC VPS install; pplx.app backend sandbox still 503, kept as secondary. See §9 + “Deployment conventions” in torii-quest-todo.md)
