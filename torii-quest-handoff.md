...
**Current version:** v0.2.529-alpha - BOOT LOADING OVERLAY + ENTRY DEEP DIVE (full-screen CSS loader with progress, deferred NPC + peer prewarm, boot timing instrumentation). Prior: v0.2.528-alpha - NAP ZONE LAYOUT FIX. See §0 for the current snapshot.
...
- **v0.2.529-alpha - BOOT LOADING OVERLAY + ENTRY DEEP DIVE.** Deep-dive analysis of the ENTER ARENA → first-frame path identified multiple bottlenecks: (1) No loading screen — just a text label on the button; the browser couldn't paint feedback during synchronous boot() work. (2) buildNapNpc() loaded 8.5MB chiefmonkey-npc-animations.glb + 1.2MB chiefmonkey6.glb during bootstrapPhysics() despite being purely cosmetic. (3) _prewarmPeerTemplates() ran full avatar builds (skeletonClone + traverse + mixer + gun attach) on MP connect during entry. (4) No timing instrumentation to diagnose entry latency. **Fixes:** (a) Full-screen CSS-only loading overlay (#boot-overlay) shown immediately on ENTER click, with progress bar + step labels; uses double-rAF yield before heavy work so the browser can paint. (b) Deferred buildNapNpc() to fire-and-forget after first visible frame (rAF, NOT setTimeout — constraint [3]). (c) Deferred _prewarmPeerTemplates() to after first frame (rAF). (d) New bootTiming.js module: performance.mark/measure for import-runtime, boot, buildArena, initAtmosphere, buildMirror, initPhysics, buildArenaColliders, loadPlayerModel, first-render. Exposed via ToriiDebug.bootTiming(). (e) Progress callbacks from arenaRuntime boot() → main.js overlay. Combat / score publishing untouched. 2668 tests pass.
...
- **Version:** v0.2.529-alpha (tag `v0.2.529-alpha`).
...
**PENDING DEPLOY:** v0.2.529-alpha is pushed to GitHub (tag `v0.2.529-alpha`) but NOT yet deployed to the VPS. The user needs to run:
```
sudo truncate -s 0 /var/log/torii-quest-update.log && sudo torii-deploy v0.2.529-alpha
```
