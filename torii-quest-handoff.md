...
**Current version:** v0.2.371-alpha (source at v0.2.371-alpha; LIVE at v0.2.370-alpha on https://chiefmonkey.art/quest/ — the v0.2.370 CSP base-aware fix deployed, so anonymous ENTER ARENA now BOOTS into the arena. Remaining open edge: players cannot see each other (H4), root-caused + fixed in this v0.2.371 source (peer-avatar GLTFLoader was missing a DRACOLoader) — pending a Suite redeploy. Torii Suite SHC VPS install; pplx.app backend sandbox still 503, kept as secondary. See §9 + “Deployment conventions” in torii-quest-todo.md)
