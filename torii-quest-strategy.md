...
Current live version: `v0.2.113-alpha` (clean source at `v0.2.364-alpha`, MP-2 build pending manual deploy)  
Multiplayer status: **MP-2 landed at v0.2.364-alpha** — server-authoritative hit resolution on the shipped MP-1 wire (`PROTOCOL_VERSION=1`, additive `RESPAWN`). See `MP_2_SPEC.md`. Prior clean source at `v0.2.144-alpha`.  
