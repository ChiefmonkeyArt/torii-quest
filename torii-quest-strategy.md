...
- **Torii Asset Forge**: validator-first prompt-to-game-ready asset pipeline (external AI presets + routstr/cashu.me credits + `torii.asset` validator/converter; not a rigging engine).
...
- **Paid with freedom-tech credits.** Pay for generation via **routstr** +
  **cashu.me credits** so the forge stays self-sovereign and has no
